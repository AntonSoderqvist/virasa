#version 450
#extension GL_EXT_buffer_reference2 : require
#extension GL_EXT_scalar_block_layout : require
#extension GL_EXT_nonuniform_qualifier : require
#extension GL_EXT_shader_explicit_arithmetic_types_int64 : require

// Height-banded terrain fragment shader (MaterialModel::TerrainHeight).
//
// Shares the forward vertex shader (cube.vert) and the 192-byte forward push
// constant layout with cube.frag. The only difference from cube.frag is that
// the surface albedo is derived from world-space elevation (fragWorldPos.z)
// in three bands — tan base, green midslope, white peaks — instead of from the
// base-color texture and PBRFactors. Lighting and hardware-PCF shadowing are
// identical to the PBR forward shader so terrain receives the same lights and
// shadows as everything else.
//
// NOTE (v1, hardcoded): the band edge elevations and colors below are fixed
// world-Z constants tuned for the procedural racing scene. A later revision
// should drive these from material parameters or the TerrainHeightField
// min/max so the bands track the actual elevation range.

// Matches virasa::LightGPU (64 bytes, scalar layout).
struct LightGPU {
    vec3  position;       // @  0
    uint  type;           // @ 12
    vec3  direction;      // @ 16
    float range;          // @ 28
    vec3  color;          // @ 32  (linear RGB * intensity)
    float innerConeCos;   // @ 44
    float outerConeCos;   // @ 48
    int   shadowIndex;    // @ 52  (-1 = unshadowed, else ShadowGPU index)
    float _pad1;          // @ 56
    float _pad2;          // @ 60
};

layout(buffer_reference, scalar) readonly buffer LightTable {
    LightGPU lights[];
};

// Matches virasa::ShadowGPU (80 bytes, scalar layout).
struct ShadowGPU {
    mat4  lightViewProj;  // @  0  (world -> light clip)
    uint  shadowMapSlot;  // @ 64  (bindless shadow-map slot, set 0 binding 1)
    float depthBias;      // @ 68
    float slopeBias;      // @ 72
    uint  _pad0;          // @ 76
};

layout(buffer_reference, scalar) readonly buffer ShadowTable {
    ShadowGPU shadows[];
};

layout(set = 0, binding = 0) uniform sampler2D textures[];
layout(set = 0, binding = 1) uniform sampler2DShadow shadowMaps[];

// 192-byte forward push constant block, identical to cube.vert / cube.frag.
layout(push_constant) uniform PushConstants {
    mat4 mvp;
    mat4 model;
    uint64_t vertexBufferAddress;
    uint64_t indexBufferAddress;
    uint64_t materialBufferAddress;
    uint64_t lightBufferAddress;
    uint64_t shadowBufferAddress;
    uint materialId;
    uint lightCount;
    vec4 highlight;
} pc;

// Hardware-PCF visibility (identical to cube.frag).
float shadowVisibility(int idx, vec3 worldPos, float NdotL) {
    if (idx < 0 || pc.shadowBufferAddress == 0ul) {
        return 1.0;
    }
    ShadowTable st = ShadowTable(pc.shadowBufferAddress);
    ShadowGPU s = st.shadows[idx];

    vec4 lc = s.lightViewProj * vec4(worldPos, 1.0);
    if (lc.w <= 0.0) {
        return 1.0;
    }
    vec3 ndc = lc.xyz / lc.w;

    float currentDepth = ndc.z;
    if (currentDepth > 1.0) {
        return 1.0;
    }

    vec2 uv = ndc.xy * 0.5 + 0.5;
    if (any(lessThan(uv, vec2(0.0))) || any(greaterThan(uv, vec2(1.0)))) {
        return 1.0;
    }

    float bias = s.depthBias + s.slopeBias * (1.0 - clamp(NdotL, 0.0, 1.0));
    float refDepth = clamp(currentDepth - bias, 0.0, 1.0);

    vec2 texel = 1.0 / vec2(textureSize(shadowMaps[s.shadowMapSlot], 0));
    float lit = 0.0;
    for (int x = -2; x <= 2; ++x) {
        for (int y = -2; y <= 2; ++y) {
            lit += texture(shadowMaps[s.shadowMapSlot],
                           vec3(uv + vec2(x, y) * texel, refDepth));
        }
    }
    return lit / 25.0;
}

// Elevation bands (world Z). Tuned for the procedural racing scene.
const vec3  kTanColor   = vec3(0.76, 0.66, 0.50);  // base / lowland
const vec3  kGreenColor = vec3(0.30, 0.55, 0.27);  // midslope
const vec3  kWhiteColor = vec3(0.95, 0.95, 0.97);  // peaks / snow
const float kTanGreenLo = 1.0;   // start blending tan -> green
const float kTanGreenHi = 3.0;   // fully green
const float kGreenWhiteLo = 6.0; // start blending green -> white
const float kGreenWhiteHi = 10.0; // fully white

vec3 heightAlbedo(float z) {
    vec3 c = mix(kTanColor, kGreenColor, smoothstep(kTanGreenLo, kTanGreenHi, z));
    c = mix(c, kWhiteColor, smoothstep(kGreenWhiteLo, kGreenWhiteHi, z));
    return c;
}

layout(location = 0) in vec2 fragUV;
layout(location = 1) in vec3 fragWorldPos;
layout(location = 2) in vec3 fragWorldNormal;
layout(location = 0) out vec4 outColor;

void main() {
    vec3 albedo = heightAlbedo(fragWorldPos.z);

    vec3 N = normalize(fragWorldNormal);

    // Small ambient term so unlit faces aren't pure black.
    vec3 lit = albedo * 0.03;

    if (pc.lightBufferAddress != 0ul && pc.lightCount > 0u) {
        LightTable lt = LightTable(pc.lightBufferAddress);
        for (uint i = 0u; i < pc.lightCount; ++i) {
            LightGPU L = lt.lights[i];

            vec3  Ldir;
            float atten = 1.0;

            if (L.type == 0u) {
                Ldir = normalize(-L.direction);
            } else {
                vec3 toLight = L.position - fragWorldPos;
                float dist = length(toLight);
                Ldir = toLight / max(dist, 1e-5);

                if (L.range > 0.0) {
                    float x = clamp(1.0 - (dist / L.range), 0.0, 1.0);
                    atten = x * x;
                } else {
                    atten = 1.0 / max(dist * dist, 1e-4);
                }

                if (L.type == 2u) {
                    float cd = dot(normalize(-L.direction), Ldir);
                    float spot = clamp(
                        (cd - L.outerConeCos) /
                        max(L.innerConeCos - L.outerConeCos, 1e-5),
                        0.0, 1.0);
                    atten *= spot;
                }
            }

            float NdotL = max(dot(N, Ldir), 0.0);
            float visibility = shadowVisibility(L.shadowIndex, fragWorldPos, NdotL);
            lit += albedo * L.color * NdotL * atten * visibility;
        }
    }

    outColor = vec4(lit, 1.0);
}
